...
exports.postSubscribeSubscription = (req, res, next) => {
  User.findById(req.user.id, (err, user) => {
    if (err) { return next(err); }
    user.subscription.title = req.body.title || '';
    user.subscription.price = req.body.price || '';
    user.subscription.discountedPrice = req.body.discountedPrice || '';
    user.subscription.discount = req.body.discount || '';
    user.save((err) => {
      if (err) {
        return next(err);
      }
      req.flash('success', { msg: 'Subscription was successful.' });
...
exports.postCancelSubscription = (req, res, next) => {
  User.findById(req.user.id, (err, user) => {
    if (err) { return next(err); }
    ...
    user.save((err) => {
      ...
      req.flash('success', { msg: 'Subscription has been cancelled.' });
      res.redirect('/account/subscription');
    });
  });
};

exports.postSwitchSubscription = (req, res, next) => {
  User.findById(req.user.id, (err, user) => {
    if (err) { return next(err); }
    user.subscription.title = req.body.title || '';
    user.subscription.price = req.body.price || '';
    ...
    user.save((err) => {
      ...
      req.flash('success', { msg: 'Subscription has been switched.' });
      res.redirect('/account/subscription');
    });
  });
};
